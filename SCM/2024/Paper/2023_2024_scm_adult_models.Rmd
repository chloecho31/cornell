...
results <- attStats(boruta)
ordered_features <- results[order(-results$medianImp), ]
ordered_features
...
model9 <- glmer.nb(total_flies ~ scale(date_collected_numeric) + 
                     cover_crop + 
                     previous_crop + 
                     scale(tmean_f) + 
                     scale(prop_corn_current_year_1000m) + 
                     ...
                     previous_crop * scale(tmean_f) + 
                     scale(tmean_f) * cover_crop + 
                     soil_texture + scale(ph) + scale(organic_matter) +
                     (1 | field_id), data = adult_features)
...
options(na.action = "na.fail")
model9_dredge <- dredge(model9)
nrow(model9_dredge)
model9_dredge[1,]

best9 <- get.models(model9_dredge, delta==0)[[1]]
summary(best9)
