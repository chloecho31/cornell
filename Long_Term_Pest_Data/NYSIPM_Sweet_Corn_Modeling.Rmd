...
cew_rf_1 <- randomForest(formula = Count ~ Lat.Decimal + Long.Decimal + Day + Year + 
                           prop_corn + Min_Temp + Max_Temp + Avg_Temp + Total_Precip,
...
X_train <- train_set %>% 
  dplyr::select(c(Lat.Decimal, Long.Decimal, Day, Year, prop_corn, Min_Temp, Max_Temp, Avg_Temp, Total_Precip)) %>%
  as.matrix()
...
X_train_thresholds <- train_set_thresholds %>% 
  dplyr::select(c(Lat.Decimal, Long.Decimal, Day, Year, prop_corn, Min_Temp, Max_Temp, Avg_Temp, Total_Precip, Min_Hum, Max_Hum)) %>%
  as.matrix()
...
cew_threshold_features_numeric %>% 
  count(Threshold)
