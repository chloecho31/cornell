...
sample_split <- sample.split(Y = ecb_e_features$Count, SplitRatio = 0.7)

train_set <- subset(x = ecb_e_features, sample_split == TRUE)
test_set <- subset(x = ecb_e_features, sample_split == FALSE)

y_train <- as.integer(train_set$Count) - 1
y_test <- as.integer(test_set$Count) - 1

X_train <- train_set %>% select(-Count)
X_test <- test_set %>% select(-Count)
