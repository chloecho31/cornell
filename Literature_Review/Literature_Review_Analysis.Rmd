...
paper_data <- papers %>% select(c(response_category, local, local_effect, landscape_type, landscape_effect, interaction)) %>%
  mutate_at(c('response_category', 'local', 'landscape_type', 'local_effect', 'landscape_effect', 'interaction'), as.factor)
paper_data
...
other <- paper_data %>% filter(response_category == 'Other')
...
other_local_positive <- sum(na.omit(other$local_effect == 'Positive'))
other_local_negative <- sum(na.omit(other$local_effect == 'Negative'))
other_local_ns <- sum(na.omit(other$local_effect == 'NS'))

other_landscape_positive <- sum(na.omit(other$landscape_effect == 'Positive'))
other_landscape_negative <- sum(na.omit(other$landscape_effect == 'Negative'))
other_landscape_ns <- sum(na.omit(other$landscape_effect == 'NS'))

other_interaction_sig <- sum(na.omit(other$interaction != 'NS'))
other_interaction_ns <- sum(na.omit(other$interaction == 'NS'))
...
local_effects_df %>%
  ...
  geom_text(data = local_strategy_counts, aes(x=1.8, y=5, label = n), vjust = -10, hjust = -1.75,
...
landscape_effects_df %>%
  ...
  facet_wrap(~ local) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = landscape_strategy_counts, aes(x=1.8, y=5, label = n), vjust = -10, hjust = -1.5,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
...
interaction_df <- paper_data %>%
  filter(interaction == 'NS' | interaction != 'NS') %>%
