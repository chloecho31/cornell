...
paper_data <- papers %>% select(c(response_category, local_strategy, local_effect, landscape_effect, minimum_complexity, maximum_complexity, 
                                  interaction_effect)) %>%
  mutate_at(c('response_category', 'local_strategy', 'local_effect', 'landscape_effect', 'minimum_complexity', 'maximum_complexity',
...
papers %>% distinct(title, .keep_all = TRUE) %>%
  ...
  mutate(paper_num = c(1:127)) %>%
  select(paper_num, minimum_complexity, maximum_complexity) %>%
  melt(id.vars = 'paper_num') %>%
  mutate_at('value', as.factor) %>%
  ggplot(aes(x = value, y = paper_num)) + 
