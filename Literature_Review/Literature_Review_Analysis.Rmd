...
paper_data <- papers %>% select(c(response_category, local_strategy, local_effect, landscape_effect, interaction_effect)) %>%
  mutate_at(c('response_category', 'local_strategy', 'local_effect', 'landscape_effect', 'interaction_effect'), as.factor)
paper_data
...
floral <- paper_data %>% filter(local_strategy == 'Floral')
diversity <- paper_data %>% filter(local_strategy == 'Diversity')
intensity <- paper_data %>% filter(local_strategy == 'Intensity')
structure <- paper_data %>% filter(local_strategy == 'Structure')
...
arthropod <- paper_data %>% filter(response_category == 'Arthropod')
crop <- paper_data %>% filter(response_category == 'Crop')
natural_enemy <- paper_data %>% filter(response_category == 'Natural enemy')
pest <- paper_data %>% filter(response_category == 'Pest')
pollinator <- paper_data %>% filter(response_category == 'Pollinator')
...
local_effects_df <- paper_data %>%
  filter(local_effect == 'Positive' | local_effect == 'Negative' | local_effect == 'NS') 
...
local_effects_df %>%
  mutate(local_effect = factor(local_effect, levels = c('Positive', 'Negative', 'NS'))) %>%
  ggplot(aes(x = local_effect, fill = local_effect, by = 1)) + 
  geom_bar() +
  ...
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = local_strategy_counts, aes(x=1.8, y=5, label = n), vjust = -10, hjust = 2,
...
landscape_effects_df <- paper_data %>%
  filter(landscape_effect == 'Positive' | landscape_effect == 'Negative' | landscape_effect == 'NS') %>%
...
landscape_effects_df %>%
  mutate(landscape_effect = factor(landscape_effect, levels = c('Positive', 'Negative', 'NS'))) %>%
  ...
  facet_wrap(~ local_strategy) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = landscape_strategy_counts, aes(x=1.8, y=5, label = n), vjust = -10, hjust = 1.75,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
...
local_effects_df %>% 
  ...
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = local_category_counts, aes(x=1.8, y=5, label = n), vjust = -10, hjust = 1.2,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
  expand_limits(y = c(0, 200)) +
  scale_fill_viridis(discrete = TRUE)
...
landscape_effects_df %>%
  ...
  facet_wrap(~ response_category) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = landscape_category_counts, aes(x=1.8, y=5, label = n), vjust = -10, hjust = 1.25,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
...
interaction_df %>%
  mutate(interaction_effect = factor(interaction_effect, levels = c('S', 'NS'))) %>%
  ggplot(aes(x = interaction_effect, fill = interaction_effect, by = 1)) + 
  geom_bar() +
  ...
  facet_wrap(~ response_category) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = interaction_category_counts, aes(x=1.8, y=5, label = n), vjust = -11, hjust = 1.75,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
...
interaction_df %>%
  ...
  facet_wrap(~ local_strategy) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = interaction_strategy_counts, aes(x=1.8, y=5, label = n), vjust = -11, hjust = -0.1,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
...
- More positive in simple landscapes 
- More negative in simple landscapes
- More positive in intermediate landscapes 
- More negative in intermediate landscapes 
- More positive in complex landscapes 
- More negative in complex landscapes 
...
paper_data %>%
  ...
  ylab('Number of Observations') +
  labs(fill = 'Interaction Effect') +
  scale_x_discrete(guide = guide_axis(angle = 45), label = abbreviate) +
  facet_wrap(~ response_category) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = interaction_category_counts, aes(x=1.8, y=5, label = n), vjust = -11, hjust = 1,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
...
paper_data %>%
  ...
  facet_wrap(~ local_strategy) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = interaction_strategy_counts, aes(x=1.8, y=5, label = n), vjust = -11, hjust = 1.5,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
