...
paper_data %>%
  ...
  ggplot(aes(x = local_effect, fill = local_effect)) + 
  geom_bar() +
...
paper_data %>%
  ...
  ggplot(aes(x = landscape_effect, fill = landscape_effect)) + 
  geom_bar() +
...
paper_data %>%
  ...
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  facet_wrap(~ response_category) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_nudge(y=0.2)) +
