...
local_effects_df %>%
  ...
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = local_category_counts, aes(x=1.8, y=5, label = n), vjust = -10, hjust = -0.85,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
...
landscape_effects %>%
  ...
  facet_wrap(~ response_category) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = landscape_category_counts, aes(x=1.8, y=5, label = n), vjust = -10, hjust = -0.75,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
  expand_limits(y = c(0, 150)) +
  scale_fill_viridis(discrete = TRUE)
...
interaction_df %>%
  ...
  facet_wrap(~ response_category) + 
  geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 1)), stat = "prop", position = position_dodge(width = 0.9), vjust = -0.5) +
  geom_text(data = interaction_category_counts, aes(x=1.8, y=5, label = n), vjust = -11, hjust = -0.1,
                    colour = "black", inherit.aes = FALSE, parse = FALSE) +
