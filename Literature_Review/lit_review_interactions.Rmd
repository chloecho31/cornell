...
model <- bglmer(response ~ landscape*local_strategy + (1|title) + (1|obs_id), data = recat, family='binomial', control=glmerControl(optimizer="bobyqa"), fixef.prior = normal(cov = diag(100, 12)))	
summary(model)
...
model_europe <- bglmer(response ~ landscape*local_strategy + (1|title) + (1|obs_id), data = recat_europe, family='binomial', control=glmerControl(optimizer="bobyqa"), fixef.prior = normal(cov = diag(100, 8)))	
summary(model_europe)

```
```{r}

cat_plot(model_europe, pred = landscape, modx = local_strategy)
...
model_na <- bglmer(response ~ landscape*local_strategy + (1|title) + (1|obs_id), data = recat_na, family='binomial', control=glmerControl(optimizer="bobyqa"), fixef.prior = normal(cov = diag(100, 12)))	
summary(model_na)
